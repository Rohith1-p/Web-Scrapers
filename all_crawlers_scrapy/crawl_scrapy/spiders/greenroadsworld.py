...
class GreenroadsworldSpider(SetuservSpider):
    name = 'greenroadsworld-product-reviews'

    def __init__(self, mongo_host, mongo_port, mongo_db, mongo_collection, document_id, env):
        super().__init__(mongo_host, mongo_port, mongo_db, mongo_collection, document_id, self.name, env)
        self.logger.info("Greenroadsworld process start")
        assert self.source == 'greenroadsworld'
    ...
    def parse_info(self, response):
        ...
        try:
            product_name = response.css('span[itemprop="name"]::text').extract_first()
        except:
            product_name = ''
        try:
    ...
    def parse_reviews(self, response):
        ...
        _res = BeautifulSoup(_res["result"], 'html.parser')
        res = _res.findAll('div', {'class': 'yotpo-review'})[1:]
        ...
        if res:
            for item in res:
                if item:
                    ...
                    _id = item.get('data-review-id')
                    review_date = dateparser.parse(item.find
                                                   ('span', {'class': 'yotpo-review-date'})
                                                   .text)
    ...
    def get_review_url(page, product_id):
        url = "https://staticw2.yotpo.com/batch?methods=%5B%7B%22method%22%3A%22reviews" \
              ...
              "%22%2C%22is_mobile%22%3Afalse%2C%22pictures_per_review%22%3A10%2C%22pid%22%3A" \
              "%22{}%22%2C%22locale%22%3A%22en%22%2C%22sort%22%3A%22date%22%2C%22direction%22" \
              "%3A%22desc%22%2C%22sort%20direction%22%3A%5B%22date%20desc%22%5D%7D%7D%5D" \
              "&app_key=wjNNWPaZGbkg4TlC8IgjgJwzQRCaJTIXHImSouV1&is_mobile=false" \
              "&widget_version=2019-05-26_11-48-25".format(page, product_id)
        return url
